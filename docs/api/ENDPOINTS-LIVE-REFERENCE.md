...
| POST | `/api/v1/admin/local-fields` | JWT | Global: admin, super-admin; Permisos: local_fields:create | Create local field. `timezone` must be an IANA timezone when `active` is omitted or `true`; it may be omitted only when `active` is `false`. | AdminGeographyService.createLocalField() | `src/admin/admin-geography.controller.ts` |
...
Mutaciones de assignments/membership/requests/post-registration pueden incrementar `authorization_context_versions` e invalidar cache. Lecturas de contexto (`AuthorizationContextService`) usan cache v4 fail-closed (`AUTH_CONTEXT_UNAVAILABLE` 503 si versión/fuente no disponibles) con rollout `AUTH_CONTEXT_CACHE_V4_ENABLED` (default on; `false` = rollback a fuente canónica) y métricas sin PII. Tras R07, la fuente canónica aplica `ClubAssignmentEffectivityPolicy.isEffective` (timezone de Campo Local) a `effective`/`active_assignment`; R07b/R07b2 aplican el mismo gate en `ClubRolesGuard` y `PermissionsGuard`; R07c lo aplica en `AuthService.setActiveClubContext` (`PATCH /auth/me/context`), listas/director de `ClubsService`, y fanout de cache de `RbacService`; timezone no clasificable → `LOCAL_FIELD_TIMEZONE_UNAVAILABLE` 503. Ver `AUTHORIZATION-CANONICAL-CONTRACT.md` (Authorization-time + Authorization context versioning). No hay endpoint de cache v4.
...
| GET | `/api/v1/clubs/:clubId/sections/:sectionId/director-succession` | JWT | Permisos: club_roles:read | Leer plan durable de sucesión (`director_succession_plans`); no crea assignment ni grant | DirectorSuccessionPlansService.getBySection() | `src/clubs/clubs.controller.ts` |
| POST | `/api/v1/clubs/:clubId/sections/:sectionId/director-succession/plans` | JWT | Permisos: club_roles:assign, club_roles:revoke | Programar plan `scheduled` idempotente; no termina al director vigente ni activa al sucesor | DirectorSuccessionPlansService.schedule() | `src/clubs/clubs.controller.ts` |
...
> [!NOTE]
> El baseline actual sigue ejecutando la sucesión inmediata en `POST .../director-succession`.
