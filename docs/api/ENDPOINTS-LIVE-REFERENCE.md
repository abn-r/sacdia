...
- `GET /camporee-events/:eventId/rubrics` y `GET /camporee-events/:eventId/scoring-targets` no exigen `camporee_events:read` en el guard: un juez activo asignado al evento también puede leer. El servicio rechaza al resto con `CAMPOREE_SCORING_FORBIDDEN`.
