...
| POST | `/api/v1/admin/master-honors/:id/recalculate` | JWT | `honors:update` | Placeholder registrado para recálculo manual por maestría. En esta etapa retorna `pending_implementation`; el job real se implementa en la fase del evaluador. | `src/admin/admin-phase-e-catalogs.controller.ts` |
