...
| POST | `/api/v1/annual-folders/templates` | JWT | `annual_folder_templates:create` | Crear template. Body soporta `name`, `club_type_id`, `ecclesiastical_year_id`, `active`, `minimum_points`, `closing_date` y exactamente un owner (`owner_union_id` o `owner_local_field_id`). Nuevas plantillas son borrador por defecto; `active=true` exige que la suma de secciones coincida con el máximo `annual_evidence_folder` del ranking efectivo. | `src/annual-folders/annual-folders.controller.ts` |
| PATCH | `/api/v1/annual-folders/templates/:templateId` | JWT | `annual_folder_templates:update` | Actualizar metadata del template, incluido `active`, `minimum_points`, `closing_date`, tipo/año y owner. Activar/publicar valida que las secciones sumen exactamente el máximo `annual_evidence_folder` resuelto por jerarquía. | `src/annual-folders/annual-folders.controller.ts` |
| POST | `/api/v1/annual-folders/templates/:templateId/sections` | JWT | `annual_folder_templates:update` | Agregar sección a template con `max_points` y `minimum_points`. Si la plantilla está activa, la suma final debe seguir coincidiendo con el máximo requerido por ranking. | `src/annual-folders/annual-folders.controller.ts` |
| PATCH | `/api/v1/annual-folders/templates/sections/:sectionId` | JWT | `annual_folder_templates:update` | Actualizar sección de template. Si la plantilla está activa, se revalida el total requerido por ranking. | `src/annual-folders/annual-folders.controller.ts` |
