...
- `GET /camporee-events/:eventId/rubrics` y `GET /camporee-events/:eventId/scoring-targets` no exigen `camporee_events:read` en el guard: un juez activo asignado al evento también puede leer. El servicio rechaza al resto con `CAMPOREE_SCORING_FORBIDDEN`. Cada scoring-target incluye `active_result_id` para que admin envíe `expected_active_result_id` en un override.
