...
- Títulos: `getScreenTitle` prueba `titleKey` → alias `NAV_KEY_BY_SCREEN_ID` → `snake_case(id)` contra `nav.items`. 2026-09-15: las 15 hojas que caían al título literal del sidebar ya tienen clave o alias; `screen-title.test.ts` exige lista vacía.
...
- `confirm-union` sigue fuera del catálogo. Sucesión anual (`clubs.succeed_director`) entra: `director-lf` / `assistant-lf` / `admin` / `super-admin`. Resuelta en `docs/audit/DECISIONS-PENDING.md`.
...
- **Catálogos dominio** (`catalogs-domain.ts`): `create/update/delete` en 13 pantallas (`catalogs:*`, `honor_categories:*`, `honors:*`, `ecclesiastical_years:*`). Visible: `admin` pierde Crear/Eliminar en años eclesiásticos (`ecclesiastical_years:{create,delete}` + `@GlobalRoles('super-admin')`). `ECCLESIASTICAL_YEARS_CREATE/DELETE` en el catálogo.
