...
- En la rama backend `codex/sacdia-admin-ios-auth`, en el rango `c09a600..ee84d2d`, están implementados `validateCredentials`, eligibility, metadata/transacción de sesión, `AdminSessionService`, JWT admin HS256 de 15 minutos, validación stateful por request, finalización transaccional AAL2, mapeo privado de errores canónicos y emisión/persistencia hash-only del challenge MFA pre-auth.
...
- El mismo rango contiene la persistencia privada de refresh rotation: refresh opaco aleatorio de 256 bits con digest SHA-256 únicamente, un token vigente por `session_id + family_id`, historial retenido hasta la expiración absoluta (mínimo 60 segundos), reutilización que revoca la familia, `Idempotency-Key` UUID y recibos AES-256-GCM con keyring separado y TTL exacto de 60 segundos. La sesión limita idle a 7 días y absoluto a 30 días. Un login pre-auth/AAL1 por MFA no recibe refresh; solo una sesión AAL2 posterior al challenge puede rotar. La migración usa el sentinel Better Auth `admin-disabled:<session_id>` y requiere el gate D2 de exclusión antes de migrar/publicar.
- No existe endpoint `/api/v1/auth/admin/*`, la migración no está desplegada ni verificada y ningún controller nuevo está publicado. Estos servicios y modelos son privados de la rama; el contrato de tokens legacy permanece intacto y la fachada no debe documentarse como live.
