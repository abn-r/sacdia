...
- **Permisos operativos de club**: solo asignaciones del año eclesiástico vigente (`EcclesiasticalYearService` por fechas en `America/Mexico_City`, no flag `active` ni `year_id` suelto) con `status=active` y transición del club `completed` o ausente. `designated` y planes de sucesión **no** aparecen en `/auth/me`. `member inactive` del año vigente sí aparece en `grants` con `operational: false` y `permissions: []` (banner, no tabs). `PATCH /auth/me/context` rechaza `inactive`/`ended`/año ajeno (`AUTH_ASSIGNMENT_YEAR_MISMATCH`) y corte pendiente (`503 CLUB_CYCLE_NOT_READY`). Años solapados → `409 ECCLESIASTICAL_YEAR_AMBIGUOUS`. El corte de año **no** blacklistea JWT; cache Redis `auth:context:v7:{userId}` con revisión de calendario en el envelope y TTL recortado al fin del periodo.
- **Inscripción anual**: la directiva con `club_members:approve` en la sección destino. `POST /users/:userId/membership/annual-enroll` responde 403 `ANNUAL_ENROLL_REQUIRES_DIRECTIVE` (D01) incluso si el guard deja pasar al dueño.
