...
- La lista móvil pide `GET /camporees?active=true&club_type_id=` de la sección activa (1=Aventureros, 2=Conquistadores, 3=Guías Mayores) y vuelve a filtrar en cliente con `includes_adventurers` / `includes_pathfinders` / `includes_master_guides`. Un camporee mixto sigue visible para cada tipo incluido. Sin sección activa no se envía `club_type_id` (catálogo territorial). El admin no envía el query.
...
- El detalle usa canvas `surfaceVariant` para que las cards blancas se separen. Identidad (nombre + chips) arriba; fechas/costo/campo en una sola SacCard; ubicación con preview 16:9. No hay banner “Detalle del Camporí” que repita las fechas. La descripción se oculta si solo repite el nombre. Segmented **Detalle / Asistentes / Eventos / Agenda** bajo el AppBar (la tab bar de la app no se toca). Detalle: identidad, facts, ubicación, inscripción de sección. Asistentes: roster, inscribir participantes, insumos/pedidos. Eventos: solo actividades con puntuación oficial y clasificación. Agenda: programa cronológico de todos los tipos de evento (competencia, espiritual, recreativo, descanso, etc.). Al tocar ubicación abre mapas externos.
- Directores, subdirectores, secretarios, secretarios-tesoreros, tesoreros y consejeros pueden ver Eventos y Agenda, y la clasificación oficial (`GET /local-camporees/:id/leaderboard` o `GET /union-camporees/:id/leaderboard`) desde Eventos. La clasificación va debajo de los eventos puntuados.
...
- En el detalle móvil, Detalle / Asistentes / Eventos / Agenda parten el contenido. Eventos y Agenda sólo para director, subdirector, secretario, tesorero, secretario-tesorero y consejero.
- La lista móvil de Eventos es mínima: icono, nombre y puntaje total, filtrada a `scoring_enabled` (o tipo `scoring` si el preview no manda el flag). Al entrar al evento se muestra el detalle completo con tipo, día/hora si la agenda está liberada, puntaje, lugar, descripción, personal asignado y horarios/bloques cuando existan.
- La pestaña Agenda lista todas las instancias (competencia, espiritual, recreativo, descanso y demás) en orden cronológico, día 1 → último día, por hora. Si un evento tiene bloques, cada bloque es una fila. Antes de `agenda_visible_from` no se muestran hora, día ni sede.
...
- **App**: con flag ON, la vista de inscripción espera el contexto de órdenes (loading/error fail-closed; no pinta el register legacy). Muestra **Cómo inscribir** (elegir miembros → pagar → comprobante → aparecen en Asistentes al aprobar) y entra a emitir la orden, donde se elige a los beneficiarios. La lista omite miembros ya inscritos (`registered`/`pending_approval`/`approved`). El admin muestra pestaña "Órdenes de pago" en el detalle del camporee local.
...
- La app móvil muestra la bandeja de evaluaciones sólo para asignaciones `primary` activas del juez autenticado y envía un ítem por rúbrica. El atajo vive en Acceso rápido y navega a `/judge-assignments` (no anidar bajo `/camporee/:camporeeId`; GoRouter lo interpretaría como id). La bandeja agrupa por evento: una card por prueba y filas tappable por club (sin CTA coral repetido). No muestra pendiente/cargado: el GET de asignaciones no trae estado de puntaje. La captura (`/camporee-events/:eventId/sections/:clubSectionId/score`) usa stepper +/−, total en barra inferior y no pinta el total en verde cuando el puntaje es 0.
