...
- **Corte de año**: cron `5 6 * * *` UTC más `OnModuleInit` de recuperación, `job_name` `ecclesiastical-year-cut`. Unidad = club + año: lock `pg_advisory_xact_lock`, ledger `club_year_transitions`. Cierra cargos **vencidos por fecha** (`ecclesiastical_years.end_date < año vigente.start_date`) a `status=ended` con `end_date` = fin del periodo saliente (conserva un `end_date` anterior más temprano). Activa planes `scheduled` de `director_succession_plans` creando director `active`; **no** activa CRA `designated` legado. Junta AV/CQ retorna a GM como `member inactive` (política T1); consejero/member siguen no inscritos en **su** sección. No llama `closeYear`, no blacklistea JWT. Queda **un** director operativo visible por seccion. Ledger e índices anuales aplicados a Neon development (2026-09-11).
...
- **Activación**: POST activa la fila `member inactive` del año actual en esa sección y, en la misma transacción, matricula la clase resuelta (`enrollment_id` en outcome `enrolled`). Si el salto de tipo es aceptado y no hay CRA base en destino, crea `member` active ahí. Si ya hay `member active` ahí, `already_enrolled`. Un cargo de director en otra sección no cuenta como inscrito aquí. No se copian cargos. Catálogo incompleto, edad insuficiente, sin sección destino o última GM: `blocked` `ANNUAL_CLASS_POLICY_UNRESOLVED`. El salto formativo AV→CQ / CQ→GM (R13–R14) sí se habilita; el retorno de directivo AV/CQ a GM (R04) no. Clases GM `max_duration_years > 1` siguen abiertas.
