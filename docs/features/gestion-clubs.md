...
## Contrato P0 de sucesión (parcial R01)
...
1. Programar escribe `director_succession_plans` (`POST .../director-succession/plans`)
   con idempotencia por `(scheduled_by_id, idempotency_key)` y `request_hash`.
2. Programar la sucesión no termina al director vigente, no activa al
...
`effective_date` se deriva del año objetivo; la base bloquea cambios al inicio
de un año con plan `scheduled`. Mientras el baseline inmediato conviva con la
superficie durable, los clientes no deben retirar el POST inmediato sin
