...
- **Reglas fuera del catálogo** — entra al catálogo solo lo que copia un decorador o una regla de servicio con match literal y bypass de super-admin:
  ...
  - Sucesión anual: UI `succeedClubSectionDirectorAction` solo `director-lf`/`assistant-lf` (`clubs/actions.ts`). El API exige `club_roles:assign` + `club_roles:revoke` + scope de club (`clubs.controller.ts:419-421`); el servicio también acepta `super-admin`/`admin` (`clubs.service.ts:1694-1700`).
...
- Oleada 2d (investidura/certificaciones, seguros/finanzas/inventario, catálogos geografía y dominio): verbos por pantalla en `investiture.ts`, `operations.ts`, `catalogs-geography.ts`, `catalogs-domain.ts`; la factoría `generic-catalogs-i18n/actions.ts` gatea por `screenId`. Tres `viewAny` corregidos con evidencia del GET: `insurance-expiring` (solo rol admin/coordinator), `catalogs-districts` (`local_fields:read`), `catalogs-divisions` (`countries:read`). Cambios visibles: `admin` pierde Crear/Eliminar en ideales/tipos de club, Eliminar en años eclesiásticos y desactivar config de investidura (todos `super-admin` en el API); coordinator pierde "Marcar investido" sin `investiture:mark_invested`. Detalle y evidencia en el plan.
