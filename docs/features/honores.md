...
El detalle de una especialidad **no inscrita** (tap desde el catalogo) usa la misma superficie quieta que los modos de trabajo: `SacTopBar` blanco (titulo = categoria), parche con `HonorBadgeImage` + `BoxFit.contain` sobre el fondo de pagina (sin mascara oval ni hero de color de categoria), chips pastel de nivel/aprobacion, hechos en `SacCard` (conteo de requisitos y material PDF via `SacPdfViewer`, no Safari) y una guia de 3 pasos "Como funciona" (inscribirse, elegir camino, enviar a revision). La CTA inferior `Inscribirme` pinta con el color de categoria. No hay `SliverAppBar` de color ni stepper numerado con circulos rellenos.
...
- `EXTERNAL`: mostrar una pantalla de trabajo (barra blanca, identidad a la izquierda: parche 52px + nombre, sin subtitulo duplicado en el top bar). El color de categoria es acento en iconos y en la CTA inferior (`Formato y evidencias`). Formato, evidencias y material viven en **una** `SacCard` agrupada estilo lista (filas + divisores inset, chevron, sin labels de accion truncados). Material y formato completado se abren con `SacPdfViewer` (no Safari). Tocar formato vacio abre el selector de archivos (PDF). La barra inferior solo aparece para enviar a revision cuando el formato y al menos una evidencia general ya estan cargados.

El perfil y el catalogo usan el mismo destino al reabrir una especialidad ya inscrita: requisitos para `IN_APP`, evidencias/formato para `EXTERNAL`, y detalle con selector solo si sigue en `UNDECIDED`. Las especialidades `APPROVED` abren el detalle historico. Tras elegir camino al inscribirse, la app reemplaza el detalle por ese mismo destino: no deja un hub intermedio de stats/preview que el perfil no muestra. En la grilla de perfil, cada especialidad muestra el estado de validacion en texto (badge pastel bajo el nombre).
