...
La **inscripción anual por directiva** (`POST /club-sections/:sectionId/annual-continuations`) no llama `ClassesService.enrollUser`. Resuelve la siguiente clase con `NextClassResolver` (historial regular de periodos con `end_date` anterior al objetivo, mismo `club_type_id`, `cross_type_enrollment=false`; siguiente `display_order` mayor, no `+1` numérico). `ClassEnrollmentPolicyService` en modo `annual` omite la investidura del predecesor inmediato del mismo tipo; prerrequisitos de otro tipo y `requires_invested_gm` siguen bloqueando. `max_duration_years > 1`, catálogo vacío, tipo agotado o siguiente clase en otra sección: `ANNUAL_CLASS_POLICY_UNRESOLVED` (D02; no se habilita cruce AV→CQ ni GM investido/multianual). Persistencia: `ClassEnrollmentWriter` en la misma transacción que activa `member`; reintento no borra progreso; `cross_type_enrollment=true` no crea membresía AV/CQ. GET de la lista rellena `suggested_class` con el mismo resolver.
