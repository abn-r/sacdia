...
17. PATCH/upload/delete/submit de progreso se rechazan con `CLASS_PROGRESS_LOCKED` si `locked_for_validation` o el enrollment está en estado terminal (`SUBMITTED`/`CLUB_APPROVED`/`COORDINATOR_APPROVED`/`FIELD_APPROVED`/`INVESTIDO`/`EXPIRED`). `IN_PROGRESS` y `REJECTED` permiten mutación. Además, esas escrituras (no la lectura GET) exigen que el `ecclesiastical_year_id` del enrollment sea el año eclesiástico vigente; si no, **403** `CLASS_PROGRESS_YEAR_NOT_OPERATIONAL`. El bypass owner no evita ese guard.
...
21. **Progresión anual sin INVESTIDO previo:** `validateDisplayOrderProgression` encuentra la inscripción de mayor `display_order` del usuario en el mismo `club_type_id` (cualquier `investiture_status`, `cross_type_enrollment = false`). `maxAllowedOrder` = ese `display_order`. Si el `start_date` del año de esa inscripción es estrictamente anterior al `start_date` del año de la nueva inscripción, `maxAllowedOrder += 1` (avance de un nivel por año calendario). Mismo año: sin avance. Salta de +2 bloqueado incluso si el año es posterior (ej. último 1, año nuevo, target 3 → `CLASS_LEVEL_TOO_HIGH`). Sin inscripción previa en ese tipo: permite sin restricción (primera asignación de post-registro). El campo `cross_type_enrollment = true` **no** crea automáticamente un miembro regular de Aventureros — el alta cruzada es independiente.
22. **members-progress incluye GM catch-up del mismo club:** `getClassMembersProgress` usa OR para devolver tanto miembros regulares de la sección (`club_section_id` exacto, `status: 'active'`) como inscripciones `cross_type_enrollment = true` de usuarios cuya asignación de rol pertenezca a cualquier sección del mismo club (`club_sections.main_club_id = params.clubId`). Asignaciones de otro club no aparecen. El DTO `ClassMemberProgress` incluye el campo `cross_type_enrollment: boolean` para que el cliente pueda mostrar un chip identificador.

## Prerrequisitos entre clases

Tabla aditiva `class_prerequisites` (`class_id`, `prerequisite_class_id`, `active`). En inscripción **explícita** (`POST /users/:userId/classes/enroll`), cada prerrequisito activo debe estar `INVESTIDO` para el usuario (cualquier año). Error: `CLASS_PREREQUISITE_NOT_MET` (403). `requires_invested_gm` sigue vigente y no se migra a esta tabla: la clase de entrada `Guía Mayor` debe tenerlo en `false` (primera inscripción GM sin investidura previa); `Guía Mayor Avanzado` e `Instructor` lo usan en `true`. Error: `CLASS_GM_INVESTITURE_REQUIRED` (403). Admin: `GET/POST/DELETE /admin/classes/:classId/prerequisites` con validación anti-ciclos (`ADMIN_CLASS_PREREQUISITE_CYCLE`).

La **inscripción anual por directiva** (`POST /club-sections/:sectionId/annual-continuations`) no llama `ClassesService.enrollUser`. `NextClassResolver` usa dos fases: sin historial regular del tipo, clase por edad (`ClassAssignmentResolver`); con historial, siguiente `display_order` del mismo tipo (no `+1` numérico). Si se agota AV o CQ, salta de tipo (AV→CQ edad ≥ 10, CQ→GM edad ≥ 16) a la sección destino activa del mismo club (`crossed_type`); cursar la última clase basta, `investiture_status` no discrimina. `ClassEnrollmentPolicyService` en modo `annual` omite la investidura del predecesor inmediato del mismo tipo; prerrequisitos de otro tipo y `requires_invested_gm` siguen bloqueando. `max_duration_years > 1`, catálogo vacío, edad insuficiente, sin sección destino o última GM: `ANNUAL_CLASS_POLICY_UNRESOLVED`. Persistencia: `ClassEnrollmentWriter` en la misma transacción que activa o crea `member` en destino; reintento no borra progreso; `cross_type_enrollment=true` (GM investido cursa AV/CQ) no crea membresía AV/CQ. GET de la lista rellena `suggested_class` con el mismo resolver e incluye graduados de tipo hacia esa sección.
