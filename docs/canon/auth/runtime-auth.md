...
- `assistant-admin` entra al panel (`ALLOWED_ADMIN_ROLES`); el backend aliasa `admin ↔ assistant-admin` y `coordinator` → zone/general + `director-lf`/`assistant-lf` (no union/dia);
