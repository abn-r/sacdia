...
Extiende las cinco tablas de relación histórica con columnas bitemporales, crea versiones de nombre tipadas y el ledger append-only de reorganizaciones/linaje. No introduce columnas de evidencia documental. El verificador read-only (`pnpm verify:institutional-hierarchy-migration`) se amplía en el mismo PR.
